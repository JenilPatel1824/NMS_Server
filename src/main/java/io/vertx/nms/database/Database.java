...
public class Database extends AbstractVerticle
{
    ...
    private static final Map<String, Set<String>> tableToCacheKeys = new ConcurrentHashMap<>();
    ...
    //Sets up an EventBus consumer to handle database queries, execute them using the PostgresSQL client,
    ...
    private CacheEntry getFromCache(String key)
    {
        ...
        {
            entry = new CacheEntry(entry.response(), entry.tables(), System.currentTimeMillis());
    ...
    // Evicts the oldest entry from the cache based on the timestamp.
    // Removes the entry from both the cache and the table-to-cache mapping.
    ...
    // Removes a cache entry's key from the table-to-cache mapping.
    // If a table no longer has any associated cache keys, it is removed from the mapping.
    // @param key    The cache key to remove.
    // @param tables The set of tables associated with the cache key.
